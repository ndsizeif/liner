	...
	haveNext:
		...
		case rune:
			...
			case esc:
				...
				goto mainLoop // we need to reset prompt
			// Unused keys
			...
			default:
				...
				// XXX vi keys in normal mode, if key is not supported, make it NOP
				if s.viMode == ViNormal {
					...
					switch viKey {
						...
						case "i": // insert mode
							...
							goto mainLoop
						case "I": // insert mode at beginning of line
							pos = 0
							s.enterViInsert(p, line, pos)
							goto mainLoop
						case "a": // insert mode
							s.enterViInsert(p, line, pos)
							goto mainLoop
						case "A": // insert mode at end of line
							pos = len(line)
							s.enterViInsert(p, line, pos)
							goto mainLoop
						case "H":
						...
						case "w": // next word // XXX test more
							if pos >= len(line) {
								s.doBeep()
								break
							}
							var char, punct, spc bool
							for {
								isSpace := unicode.IsSpace(line[pos])
								isPunct := unicode.IsPunct(line[pos])

								if (char && isPunct) || (punct && !isPunct && !isSpace)  || (spc && !isSpace) {
									s.debug.Printf("consecutive char/punct or spc \n")
									break
								}
								if !isSpace && !isPunct {
									s.debug.Printf("found char\n")
									char = true
								} else if isPunct {
									s.debug.Printf("found punct\n")
									punct = true
								} else if isSpace {
									s.debug.Printf("found spc\n")
									spc = true
								}
						...
						case "W": // next Word
							...
							for {
								...
								isSpace := unicode.IsSpace(line[pos])
								if wordFound && !isSpace {
									break // start of non-space
								}
								if !wordFound && isSpace {
									wordFound = true // flip word on a space
								}
								s.debug.Printf("looping\n")
							}
						case "e": // end of word
							if pos >= len(line) { // NOP if @ eol
								s.doBeep()
								break
							}
							var wordFound, symbolFound bool
							for {
								pos++
								if pos == len(line) { // stop iterating if eol
									break // end of line
								}
								isSpace := unicode.IsSpace(line[pos])
								isSymbol := unicode.IsPunct(line[pos])
								// check first if symbol sequence is terminated
								if symbolFound && isSpace || symbolFound && !isSymbol  {
									pos -= 1
									break
								}
								// check first if word sequence is terminated
								if wordFound && isSpace || wordFound && isSymbol {
									pos -= 1
									break
								}
								// start symbol sequence if we encounter one
								if !symbolFound && isSymbol {
									symbolFound = true
									continue
								}
								// start word sequence if not in a symbol sequence we encounter a non-space
								if !wordFound && !symbolFound && !isSpace {
									wordFound = true
									continue
								}
								s.debug.Printf("looping\n")
						...
						case "b": // prev word
							...
							if pos == len(line) {
								s.debug.Printf("starting at end of line\n")
								pos--
								break
							}
							var char,punct, spc bool
						...
						case "x": // delete character
							if pos >= len(line) { // there will be a blank character at eol that we can not delete
								s.doBeep()
							} else {
								n := len(getPrefixGlyphs(line[pos:], 1))
								line = append(line[:pos], line[pos+n:]...)
						...
						case "X": // delete previous character
							if pos <= 0 {
								s.doBeep()
							} else {
								n := len(getSuffixGlyphs(line[:pos], 1))
								if pos < len(line) {
									...
									line = append(line[:pos - n], nil...) // end of line
								}
								pos--
								s.needRefresh = true
							}
						...
						case "j": // previous matching history
							historyAction = true
							if historyStale {
								historyPrefix = s.getHistoryByPrefix(string(line))
								historyPos = len(historyPrefix)
						...
						case "p": // paste from yank buffer
							line, pos, next, err = s.yank(p, line, pos)
							goto haveNext // XXX test this goto
						case "~":  // change case
							s.debug.Printf("case change\n")
							var c rune
							if unicode.IsUpper(line[pos]) {
								c = unicode.ToLower(line[pos])
								s.debug.Printf("to lower: %v\n", string(c))
							} else if unicode.IsLower(line[pos]) {
								c = unicode.ToUpper(line[pos])
								s.debug.Printf("to upper: %v\n", string(c))
							} else {
								s.debug.Printf("nop: %v\n", string(line[pos]))
							}
							if c != 0 {
								line[pos] = c
							}
							if pos < len(line) - 1 {
								pos++ // increment postion
							}
						default:
						// continue loop if key is unsupported
						goto mainLoop
					}
					// any supported key calls a refresh and jump to mainloop
					err := s.refresh(p, line, pos)
					if err != nil {
						return "", err
					}
		...
		case action:
			// any action received while in replace, cancels replace
			if s.viMode == ViReplace {
				s.debug.Printf("action recieved while in replace mode, switching to normal\n")
				s.enterViNormal(p, line, pos)
				// goto mainLoop
			}
			switch v {
		...
		if s.needRefresh && !s.inputWaiting() { // XXX we need this
			err := s.refresh(p, line, pos)
			if err != nil {
				return "", err
			}
